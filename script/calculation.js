...
function calculate(event) {
    ...
    // local variables
    var infoString;
    var button_id = event.submitter.id;
    if (button_id === "calc") {
        scanInput();
    ...
    var counter1 = 1; // for different table parts
    var counter2 = 1; // for changing color
    for (var i = 0; i < inputArr.length; i++) {
...
function scanInput() {
    currency1 = String(document.getElementById('string1').value);
    currency2 = String(document.getElementById('string2').value);
    exRate = Number(document.getElementById('exValue').value);
}
// event handler
